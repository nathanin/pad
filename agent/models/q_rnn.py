...
class DeepQRNN(tf.keras.Model):
    ...
    def __init__(self, action_dim):
        ...
        self.lstm_cell = tf.nn.rnn_cell.LSTMCell(self.lstm_size)

        self.adv_value = tf.layers.Dense(self.adv_hidden+self.val_hidden,
            activation=nonlin)

        self.adv_arm_0 = tf.layers.Dense(self.adv_hidden, activation=nonlin)
        self.adv_arm_1 = tf.layers.Dense(self.adv_hidden, activation=nonlin)
        self.adv_pred = tf.layers.Dense(action_dim, activation=nonlin)

        self.val_arm_0 = tf.layers.Dense(self.val_hidden, activation=nonlin)
        self.val_arm_1 = tf.layers.Dense(self.val_hidden, activation=nonlin)
        self.val_pred = tf.layers.Dense(action_dim, activation=nonlin)

        self.optimizer = tf.train.RMSPropOptimizer(learning_rate=1e-4)

    def reset_lstm(self, dim0=1):
        self.lstm_state = (tf.zeros(shape=(dim0, self.lstm_size), dtype=tf.double),
                           tf.zeros(shape=(dim0, self.lstm_size), dtype=tf.double))

    def predictQ(self, state, keep_lstm_state=False):
        ...
        if keep_lstm_state:
            net, _ = self.lstm_cell(net, self.lstm_state)
        else:
            net, self.lstm_state = self.lstm_cell(net, self.lstm_state)
    ...
    def q_train(self, s_j, a_j, r_j, s_j1, is_end):
        self.reset_lstm(dim0=s_j.shape[0])
